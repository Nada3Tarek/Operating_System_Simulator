...
int process_command(int number_of_arguments, char** arguments)
{
	...
	//to make sure that the foundCommands is empty
	if(LIST_SIZE(&foundCommands)!=0)
	{//LIST_SIZE(&foundCommands)
		for(int i = 0; i <NUM_OF_COMMANDS ; i++)
		{
			LIST_REMOVE(&foundCommands, &commands[i]);
		}
	}
...

        {
           //get the number of arguments
        	if ((number_of_arguments-1) == commands[i].num_of_args)
        		return i;
        	else if(commands[i].num_of_args==-1)
        	{
        		if((number_of_arguments-1)>=1)
        		{
        			return i;
        		}
    ...
    //scenario 3
        for (int i = 0; i < NUM_OF_COMMANDS; i++)
        {
        	founada=1;
                ...
                for (const char *supcomahmed= arguments[0];*supcomahmed!= '\0'; supcomahmed++)
                {
                       if (!strchr(commands[i].name, *supcomahmed))
                        {
                    	   founada = 0;
                           break;
                        }
                }
                if (founada)//add every command matched to the list
                   	LIST_INSERT_TAIL( &foundCommands, &commands[i]);

                int size = LIST_SIZE(&foundCommands);
                if(i==NUM_OF_COMMANDS-1)//reach end of commands?
                {
                   if(size!=0)
                	  return CMD_MATCHED;
                   else
