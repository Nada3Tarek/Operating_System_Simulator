...
void sys_init_q(struct Env_Queue* q)
{	init_queue(q);//syscall needed
	return;
}

void sys_queuing(struct Env_Queue* queue){
...
void sys_dequeuing(struct Env_Queue* queue){
    struct Env* r1=dequeue(queue);
    acquire_spinlock(&ProcessQueues.qlock);
    sched_insert_ready(r1);
    release_spinlock(&ProcessQueues.qlock);
    cprintf("2");
    return;
}
	...
	case SYS_allocate_user_mem:
		...
		case SYS_sbrk:
			...
			return (uint32)sys_sbrk((uint32)a1);
					...
					break;
		case SYS_init_q:
				sys_init_q((struct Env_Queue*) a1);
				return 0;
				break;
			...
			case SYS_sched:
			            ...
			            break;
			case SYS_queuing:
						sys_queuing((struct Env_Queue*)a1);
						return 0;
						break;
			case SYS_dequeuing:
						sys_dequeuing((struct Env_Queue*)a1);
						return 0;
						break;
